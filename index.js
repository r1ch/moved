...
const claim = (resource,to) => DB.update(update(resource,to))
  ...
  .then(data=>{
    ...
    if(attr.Depth < depthHeuristic){
      console.log("We've already been here",attr.Depth)
      return attr
    }
    return DB.put(next(attr))
    .promise()
    .then(()=>{
      depthHeuristic = Math.max(attr.Depth,depthHeuristic)
      console.log("I've seen",attr.Depth)
      return attr
    })
    .catch(error=>{
        console.log("Next already exists",error,attr)
        return attr
    })
  })
  .catch(error=>{
...
const someHTML = input => {
  let response = {
    ...
    ${input.Views==1 ?
      `<h1>You found me!</h1>
      <small>I'm off now to <a href = "${input.To}">here</a></small>
      <br>
      <br>
      ...
      <h4>I've been living here since ${new Date(input.Reached).toLocaleString()}</h4>
      `
      :
      `<h3>I've moved <a href = "${input.To}">here</a></h3>
      <h4>I did used to live here, back in ${new Date(input.Reached).toLocaleString()}</h4>`
    }

    <h4>So far ${input.Views} visitor${input.Views==1?" has made it here (you!)":"s have made it here (including you)"}</h4>
    <h4>It takes ${input.Depth} ${input.Depth==1?"step":"steps"} to get this far.</h4>

    </body>
    </html>`}
    if(input.To){
      response.headers.Location = input.To
    }
    return response
}
...
const someErrorHTML = ()=>({
  statusCode: 200,
    ...
    <h1>Find the artwork</h1>
    <p>Start <a href = "/hello">here</a></p>
    <h4>Every time someone reaches the artwork, it moves</h4>
    </body>
    </html>`
})
